// index.js (FULL PRODUCTS - semua plan sesuai list kamu)
// Struktur & fungsi sama seperti versi rapih sebelumnya.
// Timpa file index.js lama dengan ini, jangan ubah ID elemen di index.html.

document.addEventListener('DOMContentLoaded', () => {
  /* ======================
     CONFIG (SEMUA PRODUK SESUAI DAFTAR)
  ====================== */
  const GENERAL_NOTES = ``;
  ...
  const PRODUCTS = [
    // Netflix
    {
      ...
      plans: [
        ...
        // list had repeated Netflix entries — keep them once (clean)
      ],
      fields: [
      ...
      terms: `Syarat Ketentuan Netlfix :
...
❝Tidak Dapat Menyambung ke Netflix, Coba Lagi.❞
atau
❝Unable to connect to Netflix. Please try again.❞
👉 Itu berarti masalah ada di perangkat TV, bukan server.
Solusi:
Tunggu koneksi internet stabil.
Coba gunakan hotspot dari HP.`
    },

    // YouTube Famhead
    {
      id: 'youtube-famhead',
      name: 'YouTube Famhead',
      uniqueCode: false,
      plans: [
        { id: 'yt-fam-1m', name: 'YouTube Famhead (1 Bulan)', desk: 'No Garansi', price: 'Rp. 10,500' }
      ],
      fields: [{ key: 'email', label: 'Email' }, { key: 'password', label: 'Password' }],
      terms: 'YouTube Famhead: No Garansi.'
    },
    ...
    // CapCut
    {
      ...
      fields: [{ key: 'email', label: 'Email' }, { key: 'password', label: 'Password' }],
      terms: 'CapCut Pro: Private akun, garansi sesuai masing-masing plan.'
    },

    // YouTube Indplan & Premium
    {
      id: 'youtube-indplan',
      name: 'YouTube Indplan',
      uniqueCode: false,
      plans: [
        { id: 'yt-ind-44d', name: 'YouTube Indplan (44 Hari)', desk: 'No Garansi, Individual Plan', price: 'Rp. 13,500' }
      ],
      fields: [{ key: 'email', label: 'Email' }, { key: 'password', label: 'Password' }],
      terms: 'YouTube Indplan: No Garansi.'
    },
    {
      id: 'youtube-prem',
      ...
      uniqueCode: false,
      plans: [
        { id: 'yt-3m', name: 'YouTube Premium (3 Bulan)', desk: 'No Garansi, Langsung 3 Bulan, Bukan Gsuite, Free Verif OTP 1x', price: 'Rp. 32,500' },
        { id: 'yt-3m-full', name: 'YouTube (3 Bulan Full Garansi)', desk: 'Full Garansi Disable & Backfree', price: 'Rp. 37,500' }
      ],
      fields: [{ key: 'email', label: 'Email' }, { key: 'password', label: 'Password' }],
      terms: 'YouTube Premium: perhatikan jenis paket.'
    },
    ...
    // Zoom Meeting Pro
    {
      ...
      fields: [{ key: 'email', label: 'Email' }, { key: 'password', label: 'Password' }],
      terms: 'Zoom Meeting Pro: private akun, full garansi.'
    },
    ...
    // Canva
    {
      ...
      fields: [{ key: 'email', label: 'Email' }, { key: 'password', label: 'Password' }],
      terms: 'Canva: cek jenis invite dan masa garansi.'
    },
  ...
  function findProductById(id) { return PRODUCTS.find(p => p.id === id) || PRODUCTS[0]; }
  function findPlan(product, planId) { return (product.plans || []).find(pl => pl.id === planId) || (product.plans && product.plans[0]); }
  ...
  function renderTerms(product) {
    termsBox.textContent = (product.terms ? (product.terms + '\n\n' + GENERAL_NOTES) : GENERAL_NOTES);
  }
  ...
  function onProductChange() {
    ...
    renderTerms(product);
    prefixInput.value = product.uniqueCode ? (product.prefix || 'AMD') : '';
  }
  ...
  function generateOutputText() {
    ...
    let out = '';
    out += '───〔 TRANSAKSI SUKSES 〕───\n';
    out += `┊・Kode Unik : ${kode}\n`;
    out += `┊・Nama Produk : ${product.name} — ${(plan && plan.name) || '-'}\n`;
    out += `┊・Harga : ${price}\n`;
    ...
    if (product.terms) out += product.terms + '\n\n';
    out += GENERAL_NOTES + '\n';
  ...
  function downloadTxt() {
    ...
    a.download = 'bukti_pembelian.txt';
    a.click();
  ...
  function previewPrint() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Bukti Pembelian</title>
      <style>body{font-family:monospace;padding:24px;white-space:pre-wrap;background:#fff;color:#000}</style>
      </head><body><pre>${escapeHtml(outputPre.textContent)}</pre></body></html>`;
  ...
  function hookEvents() {
    ...
    $('generate').addEventListener('click', generateOutputText);
    $('copyText').addEventListener('click', copyToClipboard);
    $('downloadTxt').addEventListener('click', downloadTxt);
    $('clear').addEventListener('click', clearOutput);
    $('previewPDF').addEventListener('click', previewPrint);
  }

  function init() {
    if (!productSel || !planSel || !outputPre) {
      console.error('Elemen UI tidak lengkap. Pastikan index.html memiliki elemen dengan ID yang diperlukan.');
      return;
    }
